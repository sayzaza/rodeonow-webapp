<template>
  ...
  >
    ...
    >
      <v-avatar color="grey lighten-3" size="70" class="mr-3" cover>
        <v-img
          :key="imageKey"
          :src="userImages.length > index ? userImages[index] : ''"
          contain
        />
      </v-avatar>

      <div v-if="!received" class="d-flex flex-column">
      ...
      <div v-else>
        ...
        <div class="d-flex justify-left">
          <Button
            ...
            :text="'Accept'"
            @buttonClicked="accept(item)"
          />
          <Button
            variant="secondary"
            style="width: 30% !important"
            :text="'Deny'"
            @buttonClicked="deny(item)"
          />
        </div>
      </div>
    ...
    <v-dialog v-model="deleteNotificationModal" min-width="500px">
      <v-card>
        <v-card-text>
          <span
        ...
        <v-card-actions class="d-flex justify-end">
          <v-btn variant="flat" color="grey" @click="deleteNotification"
            >Delete</v-btn
          >
          <v-btn color="primary" @click="deleteNotificationModal = false"
            >Cancel</v-btn
          >
        </v-card-actions>
      </v-card>
...
async function accept(user) {
  ...
  batch.update(doc(db, "users", userProfile.value.id), data);
  return batch.commit().then(console.log).catch(console.error);
}
...
function _removeUserKeysFromObject(dataKey, data, user) {
  data[dataKey] = Object.keys(user[dataKey])
    .filter((key) => {
      return key !== user.id && user[dataKey][user.id];
    })
    .reduce((total) => {
      let newTotal = {
        ...total,
