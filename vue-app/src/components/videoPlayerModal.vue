  ...
  <v-row justify="center">
    <v-dialog v-model="dialog" fullscreen max-height="100vh">
      ...
      >
        ...
        >
          <div
            class="vid-navigator prev-vdeo d-flex align-center justify-center"
          >
            <v-btn
          ...
          <div
            class="vid-navigator next-vdeo d-flex align-center justify-center"
          >
            <v-btn
              ...
              color="rgb(0,0,0,0.5)"
            >
              <v-icon color="white" variant="large"
          ...
          <div class="d-flex justify-center">
            <div class="mx-auto" style="position: relative">
              <div
                ...
                class="top-overlay-wrapper"
                @click.stop
              >
                <div
                  class="top-overlay justify-space-between pa-3 pt-1 text-white"
                >
                  <div class="d-flex flex-column">
                    ...
                    <span v-if="videoMeta.animal_name">
                      ...
                      <span v-if="videoMeta.animal_brand">
                        ({{ videoMeta.animal_brand }})
                      </span>
                    </span>
                  </div>
                  <div class="d-flex flex-column ml-auto mr-1 text-end">
                    <span v-if="videoMeta.score"
                      >Score: {{ videoMeta.score }}</span
                    >
              ...
              <video
                @pause="displayControlsEvent(true)"
                @ended="displayControlsEvent(true)"
                @click.prevent.stop="playPause"
                @mouseover="displayControlsEvent(true)"
                @mouseout="displayControlsEvent(false)"
                @mousemove="displayControlsEvent(true)"
                @play="onPlay"
                :key="videoKey"
                id="video"
                autoplay
              ...
              >
                ...
                <object
                  ...
                  :data="`flash-player.swf?videoUrl=${videoUrl}`"
                  width="1024"
                  height="576"
                >
                  <param
                    name="movie"
                    :value="`flash-player.swf?videoUrl=${videoUrl}`"
                  />
  ...
  setup() {
    ...
    const nextVideo = () => {
      ...
      let videoIndex = store.state.videos
        .map((vid) => vid.video_id)
        .indexOf(videoMeta.value.video_id);
      if (videoIndex < store.state.videos.length - 1)
        videoMeta.value = store.state.videos[videoIndex + 1];
    };
    ...
    onMounted(() => {
      if (videoMeta.value && videoMeta.value.video_id)
        initialSetup(videoMeta.value.video_id);
    });

    async function initialSetup(video_id) {
      videoUrl.value = "";
      videoUrl.value = await getDownloadURL(
        storageRef(storage, `videos/${video_id}.mov`)
      ).catch((error) => {
        console.error(error);
        return "";
      });
    ...
    watch(videoMeta, async (new_v, old_v) => {
      if (new_v != old_v) {
        await getView();
