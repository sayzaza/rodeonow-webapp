<template>
  ...
  >
    ...
    <v-card @click="fileInput.click()" class="d-flex pa-3 mb-6 align-center">
      <v-avatar
        size="120"
        class="mr-3"
    ...
    <div
      v-if="profile && profile.account_type == 1"
    ...
    <div
      v-if="profile && profile.account_type == 2"
      class="d-flex align-center mb-6 edit_profile"
    >
...
watch(profile, (profileValue) => {
  ...
  form.participating_events = Object.values(profileValue.events).map(
    (event) => events[event - 1]
  );
  form.favEvents = Object.values(profileValue.favorite_events).map(
    (event) => events[event - 1]
  );
  ComponentKey.value++;
});
...
async function save() {
  ...
  let data = {
    ...
    events:
      form.participating_events.map((x) => events.indexOf(x) + 1) ||
      profile.value.events ||
      [],
    favorite_events:
      form.favEvents.map((x) => events.indexOf(x) + 1) ||
