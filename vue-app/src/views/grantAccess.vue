<template>
  ...
  >
    ...
    <div v-if="['accounts', 'users'].includes(toggle)">
      ...
      >
      <span v-if="toggle == 'accounts'" class="text-body-1 text--disabled"
        >These accounts you have access to.</span
      >
    ...
    <div
      ...
      v-if="['invites', 'requests'].includes(toggle)"
    >
      <span v-if="toggle == 'invites'" class="text-body-1 text--disabled"
        >These are invitations you've sent for someone else to gain access to
        your account or received for access to someone else's account.</span
      >
      <div class="d-flex justify-center">
        <PulseLoader
      ...
      <span v-if="toggle == 'requests'" class="text-body-1 text--disabled"
        >These are requests you've sent to gain access to someone else's account
        or received for account access to your account.</span
      >
      ...
      <usersComp
        type="requests"
        :users="sentReceivedUsers"
        v-if="toggle == 'requests'"
        :received="true"
      />
      <usersComp :users="users" v-if="toggle == 'requests'" />
    </div>

    <v-dialog v-model="inviteModal" max-width="700px">
      <v-card>
        <v-card-title class="text-center text--disabled text-subtitle-1">
          <!-- <span>Invite</span> -->
        </v-card-title>
        <v-card-text>
          <v-btn variant="flat" width="100%" block @click="inviteUserByEmail"
            >Invite user By Email</v-btn
          >
          <v-divider class="my-2"></v-divider>
          <v-btn variant="flat" block @click="requestAccountAccess"
            >Request Account Access</v-btn
          >
        </v-card-text>
        <v-card-actions>
          <v-btn color="primary" block @click="inviteModal = false"
            >Cancel</v-btn
          >
        </v-card-actions>
      </v-card>
    ...
    <v-dialog v-model="inviteByEmail" min-width="700px">
      <v-card>
        <v-card-title class="text-center text--disabled text-subtitle-1">
        ...
        <v-card-text>
          ...
          <span v-if="emailNoExist" class="text-red text-caption"
            >E-mail does not exist</span
          >
        ...
        <v-card-actions>
          <v-btn color="primary" block @click="inviteByEmail = false"
            >Cancel</v-btn
          >
        </v-card-actions>
      </v-card>
    ...
    <v-dialog v-model="requestAccess" min-width="700px">
      <v-card>
        <v-card-title class="text-center text--disabled text-subtitle-1">
        ...
        <v-card-actions>
          <v-btn color="primary" block @click="requestAccess = false"
            >Cancel</v-btn
          >
        </v-card-actions>
      </v-card>
...
onMounted(() => {
  if (route.query.tab) {
    toggle.value = ["users", "accounts", "invites", "requests"][
      parseInt(route.query.tab)
    ];
  }
  try {
