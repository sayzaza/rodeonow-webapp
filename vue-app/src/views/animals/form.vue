...
async function save() {
    if (isEditing.value) {
        ...
        await updateDoc(docRef, data)
            .then(docRef => {
                console.log(`Edited successful: ${docRef}`)
                // if (!route.params.selectedAccessUser) {
                //     router.go(-1);
                // }
                // else {
                //     router.push({
...
watch(animal, async () => {
    if (animal.value !== null) {
        ...
        const { contractor_name, animalID, animal_type, contractor, created_time, id, events: animal_events, ...rest } = animal.value
...
<template>
    <v-form class="d-flex flex-column mx-auto my-6" style="max-width: 900px">
        <div class="d-flex justify-space-between mb-6">
            ...
            <!-- :loading="saving" -->
            <v-btn color="primary" @click="save" class="d-flex align-center justify-center mr-2">
                <span>Save</span>
            </v-btn>
        </div>
        ...
        <div class="d-flex align-end mb-6">
            ...
            <v-text-field v-model="form.name" density="compact" hide-no-data hide-selected hide-details
        ...
        <div class="d-flex align-center mb-6">
            ...
            ]" v-model="form.brand" density="compact" hide-no-data hide-selected placeholder="Animal's Brand"
